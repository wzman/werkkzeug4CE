...
void sGui_::SetTheme(const sGuiTheme &theme)
{
  ...
  // set font style
  sInt fontStyle;
  switch(theme.FontFlag)
  {
  case 1:
    fontStyle = sF2C_BOLD;
    break;
  case 2:
    fontStyle = sF2C_ITALICS;
    break;
  default:
    fontStyle = 0;
  }
  ...
  if (PropFont)
    PropFont->Init(theme.PropFont,theme.FontSize,fontStyle);
  else
    PropFont = new sFont2D(theme.PropFont,theme.FontSize,fontStyle);

  if (FixedFont)
    FixedFont->Init(theme.FixedFont,theme.FontSize,fontStyle);
  else
    FixedFont = new sFont2D(theme.FixedFont,theme.FontSize,fontStyle);
